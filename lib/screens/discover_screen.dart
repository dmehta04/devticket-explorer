...
import '../widgets/ring_route_card.dart';
import 'circular_route_detail_screen.dart';
...
class DiscoverScreen extends ConsumerWidget {
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 28),
            _buildSectionTitle(context, 'Ring Routes'),
            const SizedBox(height: 12),
            _buildRingRoutes(context),
            const SizedBox(height: 28),
    ...
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return _buildTransportCard(context, item);
        },
      ),
    );
  }

  Widget _buildTransportCard(BuildContext context, _TransportItem item) {
    return GestureDetector(
      onTap: () {
        Navigator.of(context).push(MaterialPageRoute(
          builder: (_) => TransportLinesScreen(
            ...
            children: [
              Row(
                children: [
                  ...
                  Icon(Icons.chevron_right, size: 18, color: item.color.withOpacity(0.5)),
                ],
              ),
              const Spacer(),
              Text(
  ...
  // Ring Routes section
  // ---------------------------------------------------------------------------
  Widget _buildRingRoutes(BuildContext context) {
    return Consumer(builder: (context, ref, _) {
      final circularAsync = ref.watch(circularRoutesProvider);
      return circularAsync.when(
        data: (routes) {
          if (routes.isEmpty) {
            return const Padding(
              padding: EdgeInsets.symmetric(horizontal: 16),
              child: Text('No ring routes found',
          ...
          return SizedBox(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return RingRouteCard(
                  line: routes[index],
                  onTap: () {
                    Navigator.of(context).push(MaterialPageRoute(
                      builder: (_) =>
                          CircularRouteDetailScreen(line: routes[index]),
                    ));
                  },
                );
              },
        ...
        error: (_, __) => const Padding(
          padding: EdgeInsets.symmetric(horizontal: 16),
          child: Text('Could not load ring routes',
              style: TextStyle(color: Colors.grey)),
        ),
      );
    });
